...
The first issue were polyphonic segments: We encountered challenges when it came to aligning the video with the transcript in cases where multiple people are singing. The issue arose as it became difficult to accurately match the speech with the corresponding audio in such scenarios. The same problem occurred when the vocals of the singer were stacked or there were background harmonies overlapping with the lead vocals. In many cases the model would get confused but find back to the correct time stamp once the vocal layering was over. This issue also happened when instruments would be very prominent such as the violin in the song “Flowers” by Miley Cyrus, which plays in the chorus. We were able to fix this issue after implementing spleeter which improved performance a lot. 

The quality of the lyrics also influences the performance. If the background vocals are prominent and distinguishable, the model will recognize them. However, if the lyrics don’t contain those parts, it might confuse the model. However, it is not always controllable to find lyrics that don’t miss parts of the lyrics such as background vocals that echo the lead vocals as heard in the last chorus of “Anti-Hero” by Taylor Swift.

Another problem was the lack of training data. We had a lack of data in the form of songs with corresponding transcripts that included start and end timestamps. This made it difficult to properly align the video with the transcript when people were singing. To overcome this challenge, alternative methods such as using audio separation techniques were used to better isolate the voices in the audio. However, having more comprehensive training data would have greatly improved the accuracy and effectiveness of the alignment process.
